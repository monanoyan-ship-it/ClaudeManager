...
async function createPhase(projectId, phaseNo, title) {
  const db = await getDb();
  const maxOrder = db.exec('SELECT COALESCE(MAX(sort_order), -1) FROM phases WHERE project_id = ?', [projectId]);
...
async function updatePhase(phaseId, updates) {
  ...
  db.run(`UPDATE phases SET ${fields.join(', ')} WHERE id = ?`, values);
  save();
}

async function deletePhase(phaseId) {
  const db = await getDb();
  db.run('DELETE FROM tasks WHERE phase_id = ?', [phaseId]);
...
async function createTask(phaseId, projectId, taskNo, title, detail, risks) {
  const db = await getDb();
  const maxOrder = db.exec('SELECT COALESCE(MAX(sort_order), -1) FROM tasks WHERE phase_id = ?', [phaseId]);
  const nextOrder = (maxOrder.length ? maxOrder[0].values[0][0] : -1) + 1;
...
async function importFromXml(projectId, xmlContent) {
  // Parse XML manually (no dependency needed for this simple structure)
  const phases = [];
